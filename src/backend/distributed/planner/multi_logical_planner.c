...
DeferErrorIfUnsupportedTableCombination(Query *queryTree)
{
	...
	foreach(joinTreeTableIndexCell, joinTreeTableIndexList)
	{
		 ...
		 * subquery, or stable or immutable function.
		 */
		if (rangeTableEntry->rtekind == RTE_RELATION ||
...
/*
 * HasRecurringTuples returns whether any part of the expression
 * is recurring in every shard query.
 */
bool
	...
	if (IsA(node, RangeTblEntry))
	{
		...
		if (rangeTableEntry->rtekind == RTE_RELATION)
		{
				...
				PartitionMethod(relationId) == DISTRIBUTE_BY_NONE)
			{
				*recurType = RECURRING_TUPLES_REFERENCE_TABLE;
				...
				/*
				 * Tuples from reference tables will recur in every shard query
				 * that includes it.
				 */
				return true;
			}
		...
		else if (rangeTableEntry->rtekind == RTE_FUNCTION)
		{
			*recurType = RECURRING_TUPLES_FUNCTION;
			...
			/*
			 * Tuples from functions will recur in every shard query that includes
			 * it.
			 */
			return true;
		}
